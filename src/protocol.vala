        ...
        public override bool deserialize_property (string property_name, out GLib.Value value, GLib.ParamSpec pspec, Json.Node property_node) {
            if (property_name == "arguments") {
                ...
                if (property_node.get_node_type () != Json.NodeType.ARRAY) {
                    Vls.Logger.warn ("lsp", "unexpected property node type for 'arguments' %s",
                ...
                property_node.get_array ().foreach_element ((array, index, element) => {
                    try {
                        ...
                        Vls.Logger.warn ("lsp", "argument %u to command could not be deserialized: %s", index, e.message);
                    }
                });
                ...
                if (property_node.get_value_type () != typeof (string)) {
                    value = "";
                    Vls.Logger.warn ("lsp", "unexpected property node type for 'commands' %s",
                             property_node.get_node_type ().to_string ());
                    return false;
                }
        ...
        public override bool deserialize_property (string property_name, out Value value, ParamSpec pspec, Json.Node property_node) {
            ...
            property_node.get_array ().foreach_element ((array, index, element) => {
                try {
                    ...
                    Vls.Logger.warn ("lsp", "argument %u could not be deserialized: %s", index, e.message);
                }
            });
            value = diags;
            return true;
    ...
    class TypeHierarchyItem : SerializableObject {
        ...
        public TypeHierarchyItem.from_symbol (Vala.TypeSymbol symbol) {
            ...
            if (symbol is Vala.Class)
                ...
                Vls.Logger.warn ("lsp", "unexpected symbol kind in type hierarchy: `%s'", symbol.type_name);
            }
