...
class Vls.ContextManager : Object {
    ...
    public void request_context_update (Jsonrpc.Client client) {
        ...
        foreach (var entry in compile_cancellables.entries)
            entry.value.cancel ();
        Logger.debug ("context", "request_context_update: requests=%d, delay=%dms",
    ...
    public bool check_update_context () {
        if (update_context_requests > 0 && get_monotonic_time () >= update_context_time_us) {
            Logger.debug ("context", "check_update_context: starting rebuild (requests=%d)", (int) update_context_requests);
            ...
            if (compile_in_progress_count > 0) {
                Logger.debug ("context", "check_update_context: %d compile(s) in progress, deferring", compile_in_progress_count);
                return true;
            }
                ...
                try {
                    ...
                    foreach (var compilation in project.get_compilations ()) {
                        if (compilation.is_stale ()) {
                            Logger.debug ("context", "check_update_context: starting async compile for %s", compilation.id);
                            ...
                            _server.scheduler.run_async.begin<Compilation.CompileResult> (() => {
                                ...
                                try {
                                    ...
                                    if (was_cancelled) {
                                        Logger.debug ("context", "async compile cancelled for %s (newer edit arrived)", compilation.id);
                                    } else {
                                        bool accepted = compilation.swap_compile_result (result);
                                        Logger.debug ("context", "async compile done for %s (swap %s)",
                                                       compilation.id, accepted ? "accepted" : "rejected: corrupted AST");
                                    }
                                } catch (Error e) {
                                    was_cancelled = compile_canc.is_cancelled ();
                                    if (!was_cancelled)
                                        Logger.warn ("context", "async compile failed: %s", e.message);
                                } finally {
                                    compile_in_progress_count--;
                            ...
                            foreach (var source_file in comp.get_project_files ()) {
                                if (newly_added.contains (source_file.filename)) {
                                    ...
                                    try {
                                        ...
                                        Logger.debug ("context", "discarding %s from DefaultProject", uri);
                                    } catch (Error e) {
                                        // just ignore
                                    }
                                }
                    ...
                    Logger.warn ("context", "failed to rebuild and/or reconfigure project: %s", e.message);
            ...
            if (reconfigured_projects) {
                ...
                foreach (var uri in orphaned_files) {
                    try {
                        ...
                        Logger.warn ("context", "failed to reopen in default project %s - %s", uri, e.message);
                        try {
                            ...
                            Logger.warn ("context", "failed to clear diagnostics for %s - %s", uri, e2.message);
                        }
                    }
                }
            }
    ...
    public void wait_for_context_update (Variant id, owned OnContextUpdatedFunc on_context_updated_func,
        ...
        if (pending_requests.has_key (req))
            Logger.warn ("context", "request %s already in pending requests, this should not happen", req.to_string ());
        else
            pending_requests[req] = new PendingRequest (req, (owned) on_context_updated_func);
        Logger.debug ("context", "queued %s — %d pending, %d compile(s) in progress",
                       req.to_string (), pending_requests.size, compile_in_progress_count);
        ...
        Idle.add (() => {
            ...
            if (compilation != null && !compilation.is_stale ()) {
                ...
                Logger.debug ("context", "satisfied %s via idle re-check (compilation not stale)", req.to_string ());
                pr.callback (false);
                return Source.REMOVE;
            }
            ...
            if (update_context_requests == 0 && compile_in_progress_count == 0) {
                pending_requests.unset (req);
                Logger.debug ("context", "satisfied %s via idle re-check (no pending updates)", req.to_string ());
                pr.callback (false);
            }
    ...
    public void cancel_request (Variant @params) {
        ...
        if (pr != null) {
            pending_requests.unset (req);
            Logger.debug ("context", "cancelled pending request %s", req.to_string ());
            pr.callback (true);
        }
    }
    ...
    void fire_pending_context_updates () {
        ...
        Logger.debug ("context", "fired %d pending context updates", fired.length);
