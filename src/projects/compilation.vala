...
class Vls.Compilation : BuildTarget {
    ...
    public Compilation (FileCache file_cache, AnalysisCache analysis_cache, string output_dir, string name, string id, int no,
        ...
        for (int arg_i = -1; (arg_i = Util.iterate_valac_args (args, out flag_name, out arg_value, arg_i)) < args.length;) {
            if (flag_name == "directory") {
                if (arg_value == null) {
                    Vls.Logger.warn ("compile", "Compilation(%s) null --directory", id);
                    continue;
                }
        ...
        for (int arg_i = -1; (arg_i = Util.iterate_valac_args (args, out flag_name, out arg_value, arg_i)) < args.length;) {
            if (flag_name == "pkg") {
                ...
                if (arg_value == null) {
                    Vls.Logger.warn ("compile", "Compilation(%s) --%s is null", id, flag_name);
                    continue;
                }

                string path = Util.realpath (arg_value, directory);
                if (!set_directory)
                    Vls.Logger.warn ("compile", "Compilation(%s) no --directory given, assuming %s", id, directory);
                if (flag_name == "vapi")
                    _output_vapi = path;
                ...
                if (arg_value == null) {
                    Vls.Logger.warn ("compile", "Compilation(%s) failed to parse argument #%d (%s)", id, arg_i, args[arg_i]);
        ...
        if (ignored_args > 0)
            Vls.Logger.debug ("compile", "Compilation(%s): ignored %d arguments", id, ignored_args);
        ...
        foreach (string? output_file in target_output_files) {
            if (output_file != null) {
                if (output.add (File.new_for_commandline_arg_and_cwd (output_file, output_dir)))
                    Vls.Logger.debug ("compile", "Compilation(%s): also outputs %s", id, output_file);
            }
        }
        ...
        if (_profile == Vala.Profile.POSIX) {
            ...
            if (_profile != Vala.Profile.GOBJECT)
                Vls.Logger.warn ("compile", "Compilation(%s) no --profile argument given, assuming GOBJECT", id);
        }
    }
    ...
    private void configure (Cancellable? cancellable = null, bool create_new_context = false) throws Error {
        ...
        if (_project_sources.is_empty) {
            Vls.Logger.debug ("compile", "Compilation(%s): will load input sources for the first time", id);
            if (input.is_empty)
                Vls.Logger.warn ("compile", "Compilation(%s): no input sources to load!", id);
            foreach (File file in input) {
                if (!dependencies.has_key (file)) {
                    try {
                        ...
                        Vls.Logger.warn ("compile", "Compilation(%s): %s", id, e.message);
                        Vala.CodeContext.pop ();
                        throw e;    // rethrow
                    }
                } else
    ...
    private void compile () throws Error {
        Vls.Logger.info ("compile", "compiling %s", id);
        Vala.CodeContext.push (code_context);
        ...
        foreach (File generated_file in _generated_sources) {
            ...
            try {
                ...
                Vls.Logger.warn ("compile", "could not add file for %s: %s - %s", id, generated_file.get_uri (), e.message);
        ...
        Vala.CodeContext.pop ();
        Vls.Logger.info ("compile", "finished compiling %s", id);
    }
    ...
    public bool is_stale () {
        ...
        foreach (TextDocument doc in _project_sources.values) {
            if (doc.last_updated > last_updated) {
                Vls.Logger.debug ("compile", "is_stale: stale due to %s (lu=%s > comp_lu=%s)",
                               doc.filename,
    ...
    public override void build_if_stale (Cancellable? cancellable = null) throws Error {
        ...
        if (needs_rebuild) {
            Vls.Logger.debug ("compile", "build_if_stale: recompiling");
            var compile_start = GLib.get_monotonic_time ();
            configure (cancellable);
            cancellable.set_error_if_cancelled ();
            Vls.Logger.debug ("compile", "compile: starting");
            compile ();
            var compile_elapsed = GLib.get_monotonic_time () - compile_start;
            Vls.Logger.info ("compile", "compile: done in %.3fs",
    ...
    public CompileResult compile_on_worker (Cancellable? cancellable = null) throws Error {
        ...
        foreach (File generated_file in _generated_sources) {
            try {
                ...
                Vls.Logger.warn ("compile", "compile_on_worker: could not add generated file for %s: %s - %s",
        ...
        if (worker_ctx.report.get_errors () > 0) {
            ...
            Vls.Logger.debug ("compile", "compile_on_worker: %s has parse errors (%d), marking corrupted",
    ...
    public bool swap_compile_result (CompileResult result) {
        if (result.has_corrupted_symbols) {
            Vls.Logger.debug ("compile", "swap_compile_result: %s has corrupted symbols, rejecting swap", id);
            has_corrupted_symbols = true;
        ...
        Vls.Logger.debug ("compile", "swap accepted: %s, files=%d", id, (int) _project_sources.size);
        return true;
    }
