...
namespace Vls.MesonIntrospection {
    ...
    internal void load_introspection_json (Json.Parser parser, string build_dir, string command,
        ...
        try {
            ...
            Logger.debug ("compile", "loading file %s ...", input_file.get_path ());
            ...
            if (e is IOError.NOT_FOUND) {
                ...
                Logger.debug ("compile", "file does not exist, fallback to %s", command_str);
                ...
                if (proc_status != 0) {
                    Logger.warn ("compile", "command `%s' in %s failed with exit code %d\n----stdout:\n%s\n----stderr:\n%s",
    ...
    internal void check_meson_version (string build_dir, Scheduler scheduler) throws Error {
        ...
        if (meson_version_proc_status != 0) {
            Logger.warn ("compile", "failed to get version, exit code %d\n----stdout:\n%s\n----stderr:\n%s",
        ...
        if (Util.compare_versions (meson_version_proc_stdout, "0.50.0") < 0) {
            Logger.warn ("compile", "meson < 0.50.0 not supported (version was '%s')", meson_version_proc_stdout);
